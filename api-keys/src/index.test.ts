...
describe("GET", () => {
	it("is missing /api-keys/", async () => {
		...
		const response = await worker.fetch(new Request("https://api-keys.example.com/a"), {
			method: "GET",
			...env,
		});
	...
	it("should return the value 2", async () => {
		...
		// Perform the GET
		const response = await worker.fetch(new Request("https://localhost/api-keys/aaaaaaaaaaa", {
			method: "GET"
		}), env);
...
describe("PUT", () => {
	...
	it("is invalid content type", async () => {
		...
		await env.API_KEYS.put("aaaaaaaaaaa", "3");
		// Perform the PUT
		const response = await worker.fetch(new Request("https://api-keys.example.com/api-keys/aaaaaaaaaaa", {
			method: "PUT",
			headers: {
				"Origin": "https://api-keys.example.com",
				"Authorization": "key1"
...
describe("getAllowedOrigins", () => {
	it("works", () => {
		...
		expect(getAllowedOrigins({ ...env, ALLOWED_ORIGINS: "https://example.com,https://example2.com" })).toEqual(["https://example.com", "https://example2.com"]);
